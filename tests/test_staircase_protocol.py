...
def test_threshold_increases_after_success():
    ...
    proto = UpDownStaircaseProtocol(
        initial_threshold=0.5, direction="up", n_up=1, step_size=0.1
    )
...
def test_threshold_decreases_after_two_failures():
    ...
    proto = UpDownStaircaseProtocol(
        initial_threshold=0.5, direction="up", n_down=2, step_size=0.1
    )
...
def test_smoothing_does_not_crash():
    proto = UpDownStaircaseProtocol(
        initial_threshold=0.5, smoothing=0.5
    )
    for i in range(20):
...
def test_reset_preserves_params():
    proto = UpDownStaircaseProtocol(
        initial_threshold=2.0, direction="down", n_up=2, n_down=3
    )
    proto.reset()
